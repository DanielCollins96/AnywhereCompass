...
export default async function SetupPage({ searchParams }: SetupPageProps) {
  ...
  const tabClass = (active: boolean) =>
    active
      ? "flex-1 rounded-full border border-[#d4af37]/50 bg-[#d4af37]/20 py-2.5 text-center text-sm text-[#f5e6c8]"
      : "flex-1 rounded-full border border-transparent bg-[#2a2218] py-2.5 text-center text-sm text-[#c4b59a]";
    ...
    <div className="flex min-h-[100dvh] flex-col bg-[#1a1410]">
      ...
      <main className="flex min-h-0 flex-1 flex-col px-4 py-4">
        <div className="mb-4 flex shrink-0 gap-2">
          <Link href="/setup" className={tabClass(view === "search")}>
            Search
          </Link>
          <Link href="/setup?view=pin" className={tabClass(view === "pin")}>
            Drop pin
          </Link>
        </div>
